...
contract SupplyChain is Supplier, Transporter, Manufacturer, Wholesaler, Distributor, Customer {
    ...
    event buyEvent(address buyer, address seller, address packageAddr, bytes signature, uint indexed now);
    event respondEvent(address buyer, address seller, address packageAddr, bytes signature, uint indexed now);
    event sendEvent(address seller, address buyer, address packageAddr, bytes signature, uint indexed now);
    event receivedEvent(address buyer, address seller, address packageAddr, bytes signature, uint indexed now);
    ...
    function getUserInfo(address _address) external view returns(
        userData memory
        ) {
    ...
    mapping(address => bool) public requestedByManufacturer;
    ...
    function buyRawMaterial(
        ...
        require(userInfo[msg.sender].role == roles.manufacturer, "Only Manufacturer can request raw material");

        // get the package contract
        address supplierAddr = RawMaterial(packageAddr).supplier();

        requestedByManufacturer[packageAddr] = true;

        // emit the event
        emit buyEvent(msg.sender, supplierAddr, packageAddr, signature, block.timestamp);
    }
